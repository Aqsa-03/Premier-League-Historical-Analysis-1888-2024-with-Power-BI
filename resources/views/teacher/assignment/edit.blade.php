...
@section('content')
    <div class="container mt-5">
        ...
        <h1><u>Edit Assignment</u></h1>

        <form action="{{ route('teacher.assignments.update',['assignment'=>$assignment->id]) }}" method="post" id="gradeForm" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Title</label>
                <input type="text" name="title" class="form-control" id="exampleFormControlInput1" value="{{ $assignment->title }}">
                <div id="title-error" class="error"></div>
            </div>

            <div class="mb-3">
                <label for="exampleFormControlTextarea1" class="form-label">Description</label>
                <textarea class="form-control" name="description" id="exampleFormControlTextarea1" rows="3" >{{ $assignment->description }}</textarea>
            ...
            <div class="mb-3">
                ...
                <select name="class_course_id" class="form-control js-select2-custom" id="exampleFormControlInput1">
                    ...
                    @foreach ($allotments as $allotment)
                        <option value="{{ $allotment->classCourse->id }}" {{ $assignment->class_course_id == $allotment->classCourse->id ? 'selected' : '' }}>{{ $allotment->classCourse->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="course_id" class="form-control js-select2-custom" id="exampleFormControlInput1">
                    ...
                    @foreach ($allotments as $allotment)
                        <option value="{{ $allotment->course->id }}" {{ $assignment->course_id == $allotment->course->id ? 'selected' : '' }}>{{ $allotment->course->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="datetime-local" name="due_date_time" class="form-control" id="exampleFormControlInput1" value="{{ $assignment->due_date_time }}">
