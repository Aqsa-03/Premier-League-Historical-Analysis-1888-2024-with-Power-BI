...
@section('content')
    <div class="container mt-5">
        ...
        <form action="{{ route('student.assignments.store') }}" method="post" id="gradeForm" enctype="multipart/form-data">
            @csrf

            <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Title</label>
                <input type="text" name="title" class="form-control" id="exampleFormControlInput1" value="{{ $assignment->title }}" readonly>
                <div id="title-error" class="error"></div>
            </div>

            <div class="mb-3">
                <label for="exampleFormControlTextarea1" class="form-label">Description</label>
                <textarea class="form-control" name="description" id="exampleFormControlTextarea1" readonly rows="3" >{{ $assignment->description }}</textarea>
                <div id="description-error" class="error"></div>
            </div>
            
            <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Class</label>
                <input type="text" name="class_id" class="form-control" id="exampleFormControlInput1" value="{{ $assignment->classCourse->name }} " readonly>
                <div id="title-error" class="error"></div>
            </div>

            <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Course</label>
                <input type="text" name="title" class="form-control" id="exampleFormControlInput1" value="{{  $assignment->course->name  }}" readonly>
                <div id="title-error" class="error"></div>
            </div>

            <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Due Date & Time</label>
                <input readonly type="datetime-local" name="due_date" class="form-control" id="exampleFormControlInput1" value="{{ $assignment->due_date_time }}">
    ...
    <script>
        $(document).ready(function() {
            $('#gradeForm').submit(function(e) {
                e.preventDefault();
                var userConfirmed = confirm('Are you sure you want to submit this assignment? You cannot make changes after submission.');
                if(userConfirmed)
                {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            window.location.href = "{{ route('student.assignments.index') }}";
                        } else {
                            window.location.href = "{{ route('student.assignments.index') }}";
                        }
                    },
